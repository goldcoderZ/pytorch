...
class TestSingleProc(DynamoDistributedSingleProcTestCase):
    ...
    def test_graph_split_inductor_transpose(self):
        ...
        self.assertTrue(any("When turning on both config.ddp_optimizer and config.optimize_ddp_lazy_compile, "
                            ...
                            "Please try to a) pad the input tensor to a static shape, or b) set "
                            "config.ddp_optimizer=False. Note that config.ddp_optimizer=False may slowdown "
                            "the performance" in msg for msg in cm.output))
